...
class Server:
    ...
    def get_hyper_index(self, index: int = None, page_size: int = 10) -> Dict:
        """ A method that returns a dictionary with the following
            key-value pairs:
        ...
        assert index is None or 0 <= index < len(indexed_data)

        data = []
        next_index = index

        # Get the data for the current page while considering deleted items
